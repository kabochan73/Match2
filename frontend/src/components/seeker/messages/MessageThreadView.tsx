  ...
  return (
    <main className="flex w-full flex-1 flex-col items-center gap-6 py-8">
      <div className="flex w-full max-w-2xl items-center justify-between">
        <Link href="/messages" className="text-sm text-gray-600">
          メッセージ一覧に戻る
        </Link>
      </div>

      <section className="flex w-full max-w-2xl flex-col gap-2">
        <Link href={`/jobs/${like.job_posting.id}`} className="w-fit">
          <h1 className="text-lg font-medium text-blue-600 underline">{like.job_posting.title}</h1>
        </Link>
        <Link href={`/companies/${like.job_posting.company.id}`} className="w-fit">
          <span className="text-sm text-blue-600 underline">{like.job_posting.company.name}</span>
        </Link>
      </section>
